...
turned out to be its own headache: the two addons' "config:rw" maps
didn't actually resolve to the same host path (one saw /config, the
other /app_config), so the hand-off file silently never arrived.
...
CONFIG_DIR = Path("/app_config/shairport-sync/config")
LOG_DIR = Path("/app_config/shairport-sync/logs")
PORT_MAP_FILE = CONFIG_DIR / "port_map.txt"

# This script's own state, not shared with the shell generator.
CASTBRIDGE_DIR = Path("/app_config/cast-bridge")
