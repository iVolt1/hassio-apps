...
# MP3 is the correct default here, not just a bandwidth/latency
# tradeoff: this worker runs ONE long-lived ffmpeg process
# (BroadcastStream) and fans its output out to whichever HTTP client
# is currently subscribed. FLAC and WAV both require a header at the
# very start of the stream for a decoder to make sense of anything
# after it (FLAC's STREAMINFO metadata block; WAV's RIFF/fmt chunk),
# and ffmpeg only ever writes that header once, when it starts. The
# FIRST subscriber gets it — but the watchdog below reconnects
# every time Cast playback goes IDLE, and every one of those
# reconnects subscribes to the SAME already-running ffmpeg mid-stream,
# never seeing that header. The Cast device then receives headerless
# audio it can't decode, buffers, times out, goes IDLE, and the
# watchdog reconnects again — a failure loop that looked like "no
# sound" but was actually "every reconnect after the first is
# undecodable by construction". MP3 doesn't have this problem: each
# frame carries its own header (sample rate/channels/etc.), so a
# decoder can join mid-stream at any frame boundary — the same reason
# Icecast/Shoutcast-style continuous relays use MP3/AAC rather than
# WAV/FLAC. Only switch back to "flac"/"wav" if you also change the
# architecture to give each new subscriber its own fresh ffmpeg
# process (or replay a cached header), not as a drop-in swap.
STREAM_FORMAT = "mp3"
FORMAT_SPECS = {
