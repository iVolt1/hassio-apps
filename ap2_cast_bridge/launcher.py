...
def start_shairport(name: str, fifo: str, port: int, sample_rate: int = 48000) -> subprocess.Popen:
...
def start_nqptp() -> subprocess.Popen:
    return subprocess.Popen(["nqptp"])
...
def main():
    ...
    try:
        while True:
            ...
            if nqptp_proc.poll() is not None:
                log.warning("nqptp died, restarting")
                nqptp_proc = start_nqptp()
