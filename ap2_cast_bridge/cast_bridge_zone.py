...
(written by an AirPlay 2 "pipe" backend shairport-sync instance, e.g.
the AirPlay 2 Multiroom Audio addon's generate_owntone_pipe_service.sh),
MP3-encoded by ffmpeg, served over HTTP, and pushed to a Google Cast
(Chromecast) device via pychromecast's standard media LOAD flow.
...
def make_stream_handler(broadcast: BroadcastStream, zone_name: str):
    class StreamHandler(BaseHTTPRequestHandler):
        def log_message(self, fmt, *args):
            log.debug("[%s] http: %s", zone_name, fmt % args)

        def do_GET(self):
            if self.path.rstrip("/") != "/stream.mp3":
...
class CastZoneWorker:
    def __init__(self, zone: dict):
        ...
        self.bitrate = zone.get("bitrate", "192k")
    ...
    # -- ffmpeg: raw PCM from the FIFO -> continuous MP3 on stdout ----
    ...
    # -- HTTP server for this zone's /stream.mp3 -----------------------
    def _start_http(self):
        handler = make_stream_handler(self._broadcast, self.name)
        self._httpd = ThreadingHTTPServer(("0.0.0.0", self.http_port), handler)
        threading.Thread(target=self._httpd.serve_forever, daemon=True,
                          name=f"http-{self.name}").start()
        log.info("[%s] Serving stream at :%d/stream.mp3", self.name, self.http_port)

    # -- Chromecast discovery + play_media + watchdog -------------------
    def _get_chromecast(self):
        # Try the host/IP launcher.py resolved at its startup discovery
        # first — fast, no network scan. Cast devices are normally on
        # static DHCP leases, but if the IP has since moved, fall back
        # to a fresh friendly-name lookup rather than getting stuck.
        if self.chromecast_host:
            casts, browser = pychromecast.get_listed_chromecasts(
                known_hosts=[self.chromecast_host]
            )
            browser.stop_discovery()
            if casts:
                return casts[0]
            log.warning("[%s] Cast host %s unreachable, falling back to name lookup",
    ...
    def _stream_url(self) -> str:
        ...
        return f"http://{ip}:{self.http_port}/stream.mp3"

    def _play(self, cast) -> bool:
        try:
            cast.wait(timeout=15)
            mc = cast.media_controller
            mc.play_media(self._stream_url(), "audio/mpeg", stream_type="LIVE",
                           title=self.name)
